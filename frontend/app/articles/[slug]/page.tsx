...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  return (
    <main className="relative min-h-screen px-4 py-16 md:pl-28 md:pr-12">
      ...
      <article className="relative z-10 mx-auto max-w-3xl border border-border/60 bg-card/70 p-8 md:p-10">
        <Link
          href="/articles"
        ...
        <div className="mt-10 space-y-12">
          {article.sections.map((section) => (
            <section key={section.heading} className="space-y-4 border-l border-border/40 pl-4 md:pl-5">
              <h2 className="font-sans text-2xl font-semibold leading-tight tracking-tight text-foreground md:text-[1.9rem]">
                {section.heading}
              </h2>
              {section.paragraphs.map((paragraph, paragraphIndex) => (
                <p key={paragraphIndex} className="font-sans text-[1.02rem] leading-8 text-foreground">
                  {paragraph}
                </p>
              ))}
