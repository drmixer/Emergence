#!/usr/bin/env python3
"""Generate deterministic technical report artifacts for a run_id."""
...
from app.services.run_reports import generate_run_technical_artifact


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate run technical report (JSON + markdown artifacts).")
    ...
    db = SessionLocal()
    try:
        payload = generate_run_technical_artifact(
            db,
