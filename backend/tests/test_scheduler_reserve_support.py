...
from app.models.models import Agent, AgentInventory, Event, GlobalResources, Law
...
def test_executable_standing_law_can_support_active_agents_without_runtime_aid_gate(session_factory, monkeypatch):
    _configure_no_reserve(
        monkeypatch,
        session_factory,
        runtime_values={
    ...
    with session_factory() as db:
        ...
        _seed_reserve(db, food="0.20", energy="2.00")
        law_id = int(law.id)
        active_agent_id = active_agent.id
    ...
    result = asyncio.run(scheduler.process_daily_consumption())
    assert result["active_fed"] == 2

    with session_factory() as db:
        refreshed_active = db.query(Agent).filter(Agent.id == active_agent_id).one()
        reserve_aid = (
            db.query(Event)
            .filter(Event.event_type == "reserve_aid", Event.agent_id == active_agent_id)
            .one()
        )
