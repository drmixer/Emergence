...
def generate_weekly_draft(
    ...
    *,
    ...
    article = ArchiveArticle(
        ...
        content_type="approachable_article",
        status_label="observational",
        evidence_completeness=("full" if str(digest.verification_state or "") == "verified" else "partial"),
        tags=[
            (f"run_id:{digest.run_id}" if str(digest.run_id or "").strip() and str(digest.run_id) != "not-set" else "run_id:not-set"),
            "season:unknown",
            "condition:weekly-digest",
            "topic:governance",
            "status:observational",
            ("evidence:full" if str(digest.verification_state or "") == "verified" else "evidence:partial"),
        ],
        linked_record_ids=[],
