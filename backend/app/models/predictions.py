...
class PredictionMarket(Base):
    ...
    )  # proposal_pass, agent_dormant, resource_goal, law_count, etc.
    ...
    resolution_event_id = Column(Integer, ForeignKey("events.id"), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('open', 'closed', 'resolved')", name="valid_market_status"
        ),
        CheckConstraint(
            "outcome IS NULL OR outcome IN ('yes', 'no')", name="valid_market_outcome"
        ),
    )
...
class PredictionBet(Base):
    ...
    id = Column(Integer, primary_key=True)
    market_id = Column(
        Integer, ForeignKey("prediction_markets.id", ondelete="CASCADE"), nullable=False
    )
    user_id = Column(
        String(100), nullable=False
    )  # IP-based or session-based anonymous user ID
